...
def tampilkan_visualisasi(df: pd.DataFrame):
    ...
    # init state sekali untuk default checkbox
    for k, v in {
        "opt_points": True,
        "opt_median": True,
        "opt_spike":  True,
        "opt_dark":   False,   # hanya untuk grafik, bukan tema app
        "viz_mode":   "Harian",
    }.items():
    ...
    if not tgl_col:
        st.warning("Kolom tanggal tidak ditemukan."); st.write(df.columns.tolist()); return

    df[tgl_col] = pd.to_datetime(df[tgl_col], errors="coerce").dt.tz_localize(None)
    df = df.dropna(subset=[tgl_col])

    y_daily = (
        df.set_index(tgl_col).resample("D").size()
    ...
    with st.sidebar:
        ...
        dark            = st.toggle("Mode gelap (grafik)", key="opt_dark",
                                    help="Hanya mengubah skema warna grafik, bukan tema aplikasi")
    ...
    if len(y)>0:
        i = int(np.argmax(y)); fig.add_trace(go.Scatter(x=[x[i]], y=[y[i]], mode="markers",
                             name="Puncak", marker=dict(size=9, symbol="star")))
        fig.add_annotation(x=x[i], y=y[i], text=f"Puncak: {_fmt(y[i])}", showarrow=True, arrowhead=2, yshift=12)
